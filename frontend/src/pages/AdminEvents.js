...
// Normalize base URL so we always end up with `<root>/events`
// and NEVER use `/api/events`
const EVENTS_BASE_URL = (() => {
  ...
  // If env mistakenly has /api, strip it
  if (base.endsWith('/api')) {
    base = base.slice(0, -4); // remove "/api"
  }

  // If it already ends with /events, use as-is
  if (base.endsWith('/events')) {
    return base;
  }

  // Otherwise call /events on the base
  return `${base}/events`;
})();
...
function AdminEvents() {
  ...
  const handleDelete = async () => {
    ...
    try {
      const headers = getAuthHeaders();
      await axios.delete(`${EVENTS_BASE_URL}/${deleteEventId}`, {
        headers
      });
